...
export default function TransactionDetailPage() {
  ...
  const similarTxs = mockTransactions
    .filter((t) => t.type === tx.type && t.id !== tx.id)
    .slice(0, 3);
          ...
          <div className="space-y-8">
            ...
            <Card className="p-8 bg-white dark:bg-neutral-900 border-neutral-200 dark:border-white/5 rounded-4xl">
              ...
              <div className="space-y-8 relative">
                ...
                />
                <TimelineStep 
                  title={tx.status === 'success' ? 'Transaction Completed' : tx.status === 'failed' ? 'Transaction Failed' : tx.status === 'pending' ? 'Processing at Bank' : 'Awaiting Provider'}
                  time={format(txDate, "h:mm a")}
            ...
            {similarTxs.length > 0 && (
              <Card className="p-8 bg-white dark:bg-neutral-900 border-neutral-200 dark:border-white/5 rounded-4xl">
                <h3 className="text-xs font-black uppercase tracking-widest text-neutral-400 mb-6">Similar Transactions</h3>
                <div className="space-y-4">
                  {similarTxs.map((similar) => (
                    <Link 
                      key={similar.id} 
                      href={`/transactions/${similar.id}`}
                      className="flex items-center justify-between p-3 rounded-xl hover:bg-neutral-50 dark:hover:bg-white/5 transition-colors"
                    >
                      <div>
                        <p className="text-sm font-bold text-neutral-900 dark:text-white">{similar.provider}</p>
                        <p className="text-xs text-neutral-500">{format(parseISO(similar.date), "MMM dd, h:mm a")}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-bold text-neutral-900 dark:text-white">₦{similar.amount.toLocaleString('en-NG')}</p>
                        <p className="text-xs text-neutral-500 capitalize">{similar.status}</p>
                      </div>
                    </Link>
                  ))}
                </div>
            ...
            {/* Need Help */}
            <Card className="p-8 bg-neutral-50 dark:bg-white/5 border-neutral-200 dark:border-white/5 rounded-4xl">
              <h3 className="text-sm font-black text-neutral-900 dark:text-white mb-2">Need Help?</h3>
              <p className="text-sm text-neutral-500 mb-4">Contact our support team for any questions about this transaction.</p>
...
function DetailItem({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      ...
      <p className={cn("text-sm font-bold text-neutral-900 dark:text-white", mono && "font-mono")}>{value}</p>
    </div>
  );
}

function TimelineStep({ title, time, status }: { title: string; time: string; status: "completed" | "active" | "pending" | "failed" }) {
  return (
    <div className="flex gap-4 relative">
      <div className={cn(
        ...
        status === 'pending' && "bg-amber-500 border-amber-500",
        status === 'failed' && "bg-rose-500 border-rose-500"
      )}>
        {status === 'completed' && <CheckCircle2 className="h-3.5 w-3.5 text-white" />}
        {(status === 'active' || status === 'pending') && <Clock className="h-3.5 w-3.5 text-white" />}
